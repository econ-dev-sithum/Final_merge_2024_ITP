...
export default function UpdateProducts() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/products/getproducts?productId=${productId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:5000/api/products/updateproduct/${formData._id}/${currentUser._id}`, {
        method: 'PUT',
        headers: {
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        <div className='flex flex-col gap-4 sm:flex-row justify-between'>
          ...
          <option value='uncategorized'>Uncategoried</option>
            ...
            <option value='Ammenities'>Ammenities</option>
